...
def create_app():
    ...
    with app.app_context():
        ...
        def serve_root_static_factory(filename):
            ...
            # Handle bcx-marketing-uplift files
            if filename.startswith('bcx-marketing-uplift/'):
                # Correct path: go up two levels from app/__init__.py to the JA directory, then into bcx-marketing-uplift
                bcx_marketing_uplift_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'bcx-marketing-uplift'))
                try:
                    return send_from_directory(bcx_marketing_uplift_dir, filename.replace('bcx-marketing-uplift/', ''))
                except NotFound:
                    app.logger.info(f"File not found in bcx-marketing-uplift: {filename} (path searched: {os.path.join(bcx_marketing_uplift_dir, filename.replace('bcx-marketing-uplift/', ''))})")
                    abort(404)
                except Exception as e:
                    app.logger.error(f"Error serving file from bcx-marketing-uplift for {filename}: {e}")
                    abort(500)
