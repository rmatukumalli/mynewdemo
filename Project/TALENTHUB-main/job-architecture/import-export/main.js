document.addEventListener('DOMContentLoaded', () => {
    ...
    async function preloadAndProcessSampleData() {
        showStatus('Loading mock sample data...', 'info');
        csvFileInput.disabled = true; 
        importButton.disabled = true; 

        const mockCsvContent = `Job Family,Job Profile,Job Level,Role Group,Role Level Name,Role Group Skills,Role Level Skills
"Engineering","Software Engineer","Entry","Frontend Development","Junior Frontend Developer","HTML; CSS; JavaScript","React; Vue.js"
"Engineering","Software Engineer","Mid","Backend Development","Mid-level Backend Developer","Python; Java; SQL","Django; Spring Boot"
"Design","UX Designer","Senior","Product Design","Senior UX Designer","User Research; Wireframing; Prototyping","Figma; Sketch"
"Marketing","Marketing Specialist","Entry","Digital Marketing","Junior Digital Marketer","SEO; SEM; Social Media","Google Analytics; HubSpot"
"HR","HR Business Partner","Mid","Talent Acquisition","Mid-level Talent Acquisition Partner","Recruitment; Interviewing; Onboarding","ATS; HRIS"`;

        try {
            const { summaryCounts, records, headers } = processCsvData(mockCsvContent);
            allJobRecords = records; 
            headersFromLastParse = headers;

            displayImportSummary(summaryCounts);
            renderTable(headers, allJobRecords); 
            showStatus('Mock job architecture data loaded and processed successfully!', 'success');
            tabsContainer.style.display = 'block'; 
            
            const fileNameDisplay = document.createElement('p');
            fileNameDisplay.textContent = "Loaded: Mock Data (5 records)";
            ...
            console.error("Error processing mock CSV:", error);
            showStatus(`Error loading mock data: ${error.message}`, 'error');
