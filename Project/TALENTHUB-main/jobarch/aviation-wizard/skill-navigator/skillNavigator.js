...
function renderModalFooter() {
    ...
    if (navigatorState.currentStep < NAVIGATOR_STEPS.length) {
        footerHTML += `<button id="navigator-next-btn" class="btn btn-primary">Next ${getIcon('ChevronRight')}</button>`;
    } else {
        footerHTML += `<button id="navigator-save-btn" class="btn btn-primary">${getIcon('Save')} Save Skills</button>`;
    }
...
function renderStep2Competencies() {
    ...
    let html = '';
    navigatorState.selectedCapabilities.forEach(capId => {
        ...
        const competenciesForCap = globalMainState.competencies.filter(comp => comp.capabilityId === capId);
...
function renderStep3Behaviors() {
    ...
    navigatorState.selectedCompetencies.forEach(compId => {
        ...
        if (!competency || !competency.behaviours || competency.behaviours.length === 0) return;

        html += `<div class="selection-section">`;
        html += `<h4 class="selection-section-title">Behaviors for: ${competency.name}</h4>`;
        competency.behaviours.forEach(behaviorString => {
...
function handleCapabilitySelect(event) {
    const capId = event.target.closest('.capability-item')?.dataset.capabilityId;
    if (!capId) return;

    const index = navigatorState.selectedCapabilities.indexOf(capId);
    if (index > -1) {
...
function handleCompetencySelect(event) {
    ...
    if (checkbox.checked) {
        if (!navigatorState.selectedCompetencies.includes(compId)) {
...
function addStepEventListeners() {
    if (navigatorState.currentStep === 1) {
        ...
        document.querySelectorAll('.capability-item').forEach(item => item.addEventListener('click', handleCapabilitySelect));
