document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCareerPaths() {
        ...
        CAREER_SWIMLANES.forEach(swimlane => {
            ...
            rolesScrollContainer.className = 'swimlane-roles-scroll-container';

            swimlane.roles.forEach((roleId, index) => {
                const isHighlighted = isRoleInHighlightedSequence(roleId);
                const nodeItem = document.createElement('div');
                nodeItem.className = 'career-path-node-item';
    ...
    function showPathAnalysis(roleId) {
        ...
        devPlanButton.innerHTML = '<i class="fas fa-route"></i> View My Development Plan';
    ...
    function initLogic(isPathfinderContext = false) {
        console.log("Initializing Career Pathing Logic. Pathfinder context:", isPathfinderContext);
        ...
        closeAnalysisBtn = document.getElementById('closeAnalysisBtn'); // Ensure this is fetched
        ...
        if (!summaryEls.name || !currentRoleDisplayAreaEl || !suggestionsGridContainerEl || !analysisSectionEl || !closeAnalysisBtn) {
            console.error("Essential Career Pathing DOM elements not found during initLogic. Aborting.");
            return; 
        }
    ...
    window.initializeCareerNavigator = function() {
        initLogic(true); 
    };
