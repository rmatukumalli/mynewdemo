...
export const renderSelectedJobSkills = (selectedJob) => {
    ...
    // --- BEGIN ADDED LOGS ---
    console.log('[jobDetailsRenderer] Populating skillMap. globalState.ontologySkills count:', globalState.ontologySkills?.length || 0);
    console.log('[jobDetailsRenderer] globalState.competencies count:', globalState.competencies?.length || 0);
    console.log('[jobDetailsRenderer] globalState.capabilities count:', globalState.capabilities?.length || 0);

    if (globalState.ontologySkills && globalState.ontologySkills.length > 0) {
        console.log('[jobDetailsRenderer] First ontologySkill:', JSON.parse(JSON.stringify(globalState.ontologySkills[0])));
    } else {
        console.log('[jobDetailsRenderer] globalState.ontologySkills is empty or undefined.');
    }
    if (globalState.competencies && globalState.competencies.length > 0) {
        console.log('[jobDetailsRenderer] First competency:', JSON.parse(JSON.stringify(globalState.competencies[0])));
    } else {
        console.log('[jobDetailsRenderer] globalState.competencies is empty or undefined.');
    }
    if (globalState.capabilities && globalState.capabilities.length > 0) {
        console.log('[jobDetailsRenderer] First capability:', JSON.parse(JSON.stringify(globalState.capabilities[0])));
    } else {
        console.log('[jobDetailsRenderer] globalState.capabilities is empty or undefined.');
    }
    // --- END ADDED LOGS ---

    globalState.ontologySkills.forEach(skill => {
        // --- BEGIN UNCOMMENTED LOGS INSIDE LOOP ---
        console.log(`[jobDetailsRenderer] Processing skill for skillMap: ID=${skill.id}, Name="${skill.name}", CompetencyID=${skill.competencyId}`);
        // --- END UNCOMMENTED LOGS INSIDE LOOP ---

        const competency = globalState.competencies.find(c => c.id === skill.competencyId);
        
        // --- BEGIN UNCOMMENTED LOGS INSIDE LOOP ---
        if (!competency) {
            console.warn(`[jobDetailsRenderer] Competency not found for skill ID ${skill.id} (Name: "${skill.name}") using competencyId "${skill.competencyId}"`);
        } else {
            console.log(`[jobDetailsRenderer] Found competency for skill ID ${skill.id} (Name: "${skill.name}"): CompID=${competency.id}, Name="${competency.name}", CapID=${competency.capabilityId}`);
        }
        // --- END UNCOMMENTED LOGS INSIDE LOOP ---

        const capability = competency ? globalState.capabilities.find(cap => cap.id === competency.capabilityId) : null;
        
        // --- BEGIN UNCOMMENTED LOGS INSIDE LOOP ---
        if (competency && !capability) {
            console.warn(`[jobDetailsRenderer] Capability not found for competency ID ${competency.id} (Name: "${competency.name}") using capabilityId "${competency.capabilityId}"`);
        } else if (capability) {
            console.log(`[jobDetailsRenderer] Found capability for competency ID ${competency.id} (Name: "${competency.name}"): CapID=${capability.id}, Name="${capability.name}"`);
        }
        // --- END UNCOMMENTED LOGS INSIDE LOOP ---
    ...
    if (activeJobDetailTab === 'skills') {
        ...
        if (selectedJob.rawAiSkills && selectedJob.rawAiSkills.length > 0) {
            ...
            // Create a set of names from already mapped skills for efficient lookup
            const mappedSkillNames = new Set(mappedSkills.map(s => s.name.toLowerCase()));

            // Filter rawAiSkills to only include those not already in mappedSkills
            const skillsToDisplayAsAiGenerated = selectedJob.rawAiSkills.filter(
                rawSkill => !mappedSkillNames.has(rawSkill.name.toLowerCase())
            );

            // Only proceed if there are AI-generated skills to display after filtering
            if (skillsToDisplayAsAiGenerated.length > 0) {
                skillsToDisplayAsAiGenerated.forEach((aiSkill, index) => {
                ...
                `;
            }); // Closes skillsToDisplayAsAiGenerated.forEach
            } else {
                 html += `<p class="text-sm text-gray-500 italic py-4 text-center">All AI-suggested skills have been mapped or recognized in the ontology.</p>`;
            } // Closes if (skillsToDisplayAsAiGenerated.length > 0)
        }
