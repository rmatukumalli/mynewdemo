...
export const handleMapAiSkillToOntology = (rawSkillName, rawSkillProficiencyName) => {
    ...
    console.log(`[handleMapAiSkillToOntology] Attempting to map AI skill: "${rawSkillName}" (Proficiency from AI: "${rawSkillProficiencyName}")`);

    // Ensure globalState.aiProficiencyNameToOntologyLevel is accessible or define it locally if not already global
    // For this example, assuming it's available via globalState or defined in skillNavigator and imported if needed.
    // If it's only in skillNavigator.js, we might need to duplicate it or import it here.
    const proficiencyMap = { // Duplicating for clarity, ideally this would be shared
        'basic': 2, 
        'foundational': 1, 
        'intermediate': 3, 
        'advanced': 4, 
    ...
    const foundOntologySkill = globalState.ontologySkills.find(
        s => s.name.toLowerCase() === rawSkillName.toLowerCase()
    );

    if (foundOntologySkill) {
        console.log(`[handleMapAiSkillToOntology] Direct match found in ontology: ID ${foundOntologySkill.id}, Name: ${foundOntologySkill.name}`);
        const numericalProficiency = proficiencyMap[rawSkillProficiencyName.toLowerCase()] || 3; // Default to Intermediate (level 3)
        ...
        const updatedRoleGroups = globalState.roleGroups.map(group => ({
            ...
            jobs: group.jobs.map(job => {
                if (job.id === localState.selectedJobIdForSkillMapping) {
                    ...
                    if (!newSkills.some(s => s.skillId === foundOntologySkill.id)) {
                        newSkills.push({ skillId: foundOntologySkill.id, proficiency: numericalProficiency });
                        console.log(`[handleMapAiSkillToOntology] Added skill "${foundOntologySkill.name}" with proficiency ${numericalProficiency} to job.`);
                    } else {
                        console.log(`[handleMapAiSkillToOntology] Skill "${foundOntologySkill.name}" already mapped to job. Proficiency not updated by quick map.`);
                    }

                    const updatedRawAiSkills = (job.rawAiSkills || []).filter(
                        rs => rs.name.toLowerCase() !== rawSkillName.toLowerCase()
                    );
                    console.log(`[handleMapAiSkillToOntology] Removed "${rawSkillName}" from rawAiSkills. New rawAiSkills count: ${updatedRawAiSkills.length}`);
                    
                    return { ...job, skills: newSkills, rawAiSkills: updatedRawAiSkills };
                }
                return job;
            })
        }));
        ...
        globalHandlers.triggerRender();
        alert(`Skill "${rawSkillName}" was found in the ontology and has been mapped to the job with proficiency level ${numericalProficiency}. Associated behaviours can be added by editing the skill if needed.`);

    } else {
        console.log(`[handleMapAiSkillToOntology] No direct match for "${rawSkillName}" in ontology. Opening full Skill Navigator.`);
        alert(`Skill "${rawSkillName}" was not found as an exact match in the skill ontology. The Skill Navigator will open to help you find a suitable match or alternative.`);
        openSkillNavigator(
            localState.selectedJobIdForSkillMapping,
            rawSkillName, 
            rawSkillProficiencyName,
            true 
        );
    }
};
