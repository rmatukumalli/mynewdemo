...
const stepHeader = (title, subtitle) => `<div class="mb-6"><h2 class="text-2xl font-bold text-gray-800">${title}</h2><p class="text-gray-600 mt-1">${subtitle}</p></div>`;
const emptyState = (text) => `<p class="text-center text-gray-500 py-8">${text}</p>`;
    ...
    switch (state.currentStep) {
        case 1: // Organization
            ...
            contentHTML = `
                <div class="flex justify-between items-center mb-6">
                    ${stepHeader('Step 1: Define Organization', 'Set up your company or institution details.')}
                ...
                </div>` : emptyState('No organizations added. Click "Add Organization" to get started.')}
            `;
            break;
        ...
        case 3: // Department
            ...
            if (state.departments.length > 0) {
                ...
                departmentsGroupedHTML = Object.values(groupedByBU).map(buGroup => {
                    ...
                    return `
                        <div class="mb-8">
                            <h3 class="text-xl font-semibold text-gray-700 mb-3">${buGroup.name}</h3>
                            <div class="overflow-x-auto bg-white rounded-lg shadow border">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
        ...
        case 4: // Role Groups (was 1)
            contentHTML = `
                ...
                <div class="space-y-4">${state.roleGroups.length > 0 ? state.roleGroups.map(g => listItem(g, 'group', openModalFn, deleteItemFn)).join('') : emptyState('No role groups added.')}</div>`;
            break;
        case 5: // Job Levels (was 2)
            contentHTML = `
                ...
                <div class="space-y-4">${state.jobLevels.length > 0 ? state.jobLevels.map(l => listItem(l, 'level', openModalFn, deleteItemFn)).join('') : emptyState('No job levels defined.')}</div>`;
            break;
