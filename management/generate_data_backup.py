...
from app.models import (
    ...
    ProjectOrAssignment, Certification, Feedback, # Added new models
    ...
    for user in users:
        ...
        for i, skill in enumerate(gapped_skills):
            ...
            data.append({
                ...
                "proficiency_level_id": proficiency_level_id # Add this to link to a valid proficiency
            })
    return data
...
def populate_database():
    ...
    with app.app_context():
        ...
        for table_name in insertion_order_phase2_final:
            records = staged_generated_data.get(table_name)
            if not records:
                print(f"No data generated for {table_name}. Skipping insertion.")
                continue
            ...
            # Handle association tables via relationships
            if table_name == "CAPABILITY_COMPETENCY_MAP":
                for record in records:
                ...
                db.session.commit()
                print(f"Successfully processed association table {table_name}.")
                continue
            elif table_name == "COMPETENCY_SKILL_MAP":
                for record in records:
                    comp_obj = Competency.query.get(record['competency_id'])
                ...
                print(f"Successfully processed association table {table_name}.")
                continue
            elif table_name == "ROLE_COMPETENCY_MAP":
                for record in records:
                ...
                print(f"Successfully processed association table {table_name}.")
                continue
            elif table_name == "JOB_PROFILE_REQUIRED_SKILLS_MAP":
                for record in records:
                ...
                print(f"Successfully processed association table {table_name}.")
                continue
            elif table_name == "BUSINESS_UNIT_DEPARTMENTS_MAP":
                for record in records:
                ...
                print(f"Successfully processed association table {table_name}.")
                continue
            elif table_name == "LEARNING_PATH_RESOURCES":
                for record in records:
                    lp_obj = LearningPath.query.get(record['learning_path_id'])
                    res_obj = LearningResource.query.get(record['learning_resource_id'])
                    if lp_obj and res_obj:
                        lp_obj.resources.append(res_obj)
                    else:
                        print(f"Warning: Could not map learning_path_id {record['learning_path_id']} or learning_resource_id {record['learning_resource_id']} for LEARNING_PATH_RESOURCES.")
                db.session.commit()
                print(f"Successfully processed association table {table_name}.")
                continue
            else: # Default handling for other models
                Model = model_map.get(table_name)
                if not Model:
                    print(f"Warning: Model for table {table_name} not found in map. Skipping.")
                    continue

                # Get the list of valid column names for the current model
                valid_columns = Model.__table__.columns.keys()
                
                # Create a new dictionary with only the valid columns
                record_to_insert = {k: v for k, v in record.items() if k in valid_columns}

                # Handle specific type conversions for SQLAlchemy
                if table_name == "PROJECT_OR_ASSIGNMENT":
                    if 'start_date' in record_to_insert and isinstance(record_to_insert['start_date'], str):
                        record_to_insert['start_date'] = datetime.date.fromisoformat(record_to_insert['start_date'])
                    if 'end_date' in record_to_insert and isinstance(record_to_insert['end_date'], str):
                        record_to_insert['end_date'] = datetime.date.fromisoformat(record_to_insert['end_date'])
                elif table_name == "CERTIFICATION":
                    if 'issue_date' in record_to_insert and isinstance(record_to_insert['issue_date'], str):
                        record_to_insert['issue_date'] = datetime.date.fromisoformat(record_to_insert['issue_date'])
                    if 'expiry_date' in record_to_insert and isinstance(record_to_insert['expiry_date'], str):
                        record_to_insert['expiry_date'] = datetime.date.fromisoformat(record_to_insert['expiry_date'])
                elif table_name == "FEEDBACK":
                    if 'feedback_date' in record_to_insert and isinstance(record_to_insert['feedback_date'], str):
                        record_to_insert['feedback_date'] = datetime.date.fromisoformat(record_to_insert['feedback_date'])
                
                # General datetime and JSON conversions
                for key in ['created_at', 'updated_at']:
                    if key in record_to_insert and isinstance(record_to_insert[key], str):
                        record_to_insert[key] = datetime.datetime.fromisoformat(record_to_insert[key])
                
                if 'custom_fields' in record_to_insert and isinstance(record_to_insert['custom_fields'], str):
                    record_to_insert['custom_fields'] = json.loads(record_to_insert['custom_fields'])

                new_record = Model(**record_to_insert)
                db.session.add(new_record)
            db.session.commit() # Commit after each table to reduce transaction size and aid debugging
            print(f"Successfully inserted records into {table_name}.")
